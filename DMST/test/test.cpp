//refer to: http://edmonds-alg.sourceforge.net/
...
main(int argc, char *argv[])
{
    ...
    while (n_tests--)
    {
...

        {
            if (1)
            {
                cout << "Show Matrix weights:\n";
                for (int i = 0; i < n_vertices; ++i)
                {
                    for (int j = 0; j < n_vertices; ++j)
                    {
                        //weights[i][j] = i * n_vertices + j;
                        /* code */
                        cout << weights[i][j] << "\t";
                    }
        ...
        {
            int weight = 0;
            BOOST_FOREACH (Edge e, all_branchings[MIN_SPAN_0])
            {
                cout << "(" << source(e, g) << ", " << target(e, g) << ")\t";
                weight += weights[source(e, g)][target(e, g)];
            }
